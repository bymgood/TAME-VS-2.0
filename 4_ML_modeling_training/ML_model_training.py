...
def RF_ROC_plot(rf_grid_imba, X, y, X_test, y_test):
    ...
    plt.clf()
    with open('rf_classification_report.csv', 'w', encoding='utf-8') as file:
             print(classification_report_imbalanced(y_test, rf_grid_imba.predict(X_test)), file=file)
...
def MLP_ROC_plot(mlp_grid_imba, X, y, X_test, y_test):
    ...
    plt.clf()
    with open('mlp_classification_report.csv', 'w', encoding='utf-8') as file:
             print(classification_report_imbalanced(y_test, mlp_grid_imba.best_estimator_.predict(X_test)), file=file)
