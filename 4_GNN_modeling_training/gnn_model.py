...
class GATConv(MessagePassing):
    def __init__(self,
        ...
        self.edge_proj = Parameter(torch.Tensor(emb_dim + self.edge_dim, emb_dim))   # [emb(out)+edge_dim] x emb(out)  # new
    ...
    def update(self, aggr_out):   
        ...
        aggr_out = torch.mm(aggr_out, self.edge_proj)
...
class GNN(torch.nn.Module):
    ...
    """
    def __init__(self, num_layer, emb_dim, output_dim, JK="last", drop_ratio=0, gnn_type="gcn"):
        ...
        if self.JK == "concat":
           self.fc1 = nn.Linear(emb_dim * (self.num_layer + 1), emb_dim)
        else:
           self.fc1 = nn.Linear(emb_dim, emb_dim)
           
        self.fc2 = torch.nn.Linear(emb_dim, output_dim)  # Fixed for binary classification
    ...
    def forward(self, data):
        ...
        if self.JK == "concat":
            ...
            node_representation, _ = torch.max(h_stack, dim=0)
        elif self.JK == "sum":
            h_stack = torch.stack(h_list, dim=0) 
        ...
        ### Classifier
        graph_representation = F.relu(self.fc1(graph_representation))
        out = self.fc2(graph_representation)
