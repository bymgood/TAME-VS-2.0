...
class MoleculeDataset(Dataset):
    def __init__(self, data_file):
        ...
        for idx, row in df.iterrows():
            if len(row) < 2:
                print(f"Warning: Insufficient columns in row {idx+1}, skipping")
                continue
            smiles, label = row[0].strip(), row[1].strip()            
    ...
    def __getitem__(self, idx):
        ...
        label = torch.tensor([float(label)], dtype=torch.long)
        # print('smiles', smiles, 'label', label)
        mol_graph = MolGraph(smiles, label)
        data = Data(x=mol_graph.x, edge_index=mol_graph.edge_index, edge_attr=mol_graph.edge_attr, y=mol_graph.y)  
        return data
