...
class MoleculeDataset(Dataset):
    def __init__(self, data_file):
        ...
        for idx, row in df.iterrows():
            ...
            smiles = row[0].strip()           
            cmpd_id = row[1].strip()
            self.data.append((smiles, cmpd_id))
    ...
    def __getitem__(self, idx):
        ...
        smiles, cmpd_id = self.data[idx]
        mol_graph = MolGraph(smiles)
        data = Data(x=mol_graph.x, edge_index=mol_graph.edge_index, edge_attr=mol_graph.edge_attr)  
        data.smiles = smiles
        data.cmpd_id = cmpd_id
        return data
